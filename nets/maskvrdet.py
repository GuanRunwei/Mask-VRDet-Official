...
class MaskVRDet(nn.Module):
    ...
    def forward(self, x):
        fpn_outs, seg_outputs    = self.backbone.forward(x)
        det_outputs     = self.head.forward(fpn_outs)
        return det_outputs, seg_outputs


if __name__ == '__main__':
    model = MaskVRDet(num_classes=4, phi='l', is_attention=0, neck=2, num_seg_classes=5).cuda()
    input_map = torch.randn((1, 3, 512, 512)).cuda()
    t1 = time.time()
    test_times = 500
    for i in range(test_times):
        output_map, output_seg = model(input_map)
    t2 = time.time()
