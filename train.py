...
if __name__ == "__main__":
    ...
    phi = 'l'
    # ------------------------------------------------------#
    #   所使用的attention版本，0为不使用，1为使用基于卷积的shuffle attention, 2为使用context attention
    # ------------------------------------------------------#
    attention_v = 0
    # ------------------------------------------------------#
    #   所使用的用于特征融合的neck，1为使用panet，2为使用fpn
    # ------------------------------------------------------#
    neck_v = 1
    # ------------------------------------------------------------------#
    #   mosaic              马赛克数据增强。
    ...
    #   创建yolo模型
    # ------------------------------------------------------#
    model = MaskVRDet(num_classes=num_classes, num_seg_classes=num_classes_seg, phi=phi, is_attention=attention_v,
                      neck=neck_v)
    weights_init(model)
    if model_path != '':
        # ------------------------------------------------------#
